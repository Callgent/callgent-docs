...
Now let's create the `recruiting` callgent, and import the API definition into the callgent.
...
**Server endpoint** defines a group of methods that encapsulate and forward the request to the actual service used to accomplish your tasks. In order to make SEP work, we have to upload a restapi definition doc which is defined in the OpenAPI specification.
The definition api doc we just have defined in the previous step (see [Define user API](./define-user-api)).
